...
class Golf extends Bookable
{
    use Notifiable;
    use SoftDeletes;
    use CapturesService;
    protected $table = 'bravo_golfs';
    public $type = 'golf';
    public $checkout_booking_detail_file       = 'Golf::frontend/booking/detail';
    public $checkout_booking_detail_modal_file = 'Golf::frontend/booking/detail-modal';

    public    $set_paid_modal_file                = 'Golf::frontend/booking/set-paid-modal';
    public $email_new_booking_file             = 'Golf::emails.new_booking_detail';
    protected $translation_class = GolfTranslation::class;

    public $intervalTimeSlot = 20;
    ...
    protected $golfTermClass;
    protected $golfTranslationClass;
    protected $userWishListClass;
    protected $locationClass;

    protected $tmp_price = 0;
    protected $tmp_dates = [];
    protected $tmp_time_slot = [];
    ...
    public function __construct(array $attributes = [])
    {
        ...
        $this->golfDateClass = GolfDate::class;
        $this->golfTermClass = GolfTerm::class;
        $this->golfTranslationClass = GolfTranslation::class;
    ...
    {
        $meta['seo_title'] = __("Search for Golfs");
        if (!empty($title = setting_item_with_lang("golf_page_list_seo_title",false))) {
            $meta['seo_title'] = $title;
        }else if(!empty($title = setting_item_with_lang("golf_page_search_title"))) {
            $meta['seo_title'] = $title;
        }
        $meta['seo_image'] = null;
        if (!empty($title = setting_item("golf_page_list_seo_image"))) {
            $meta['seo_image'] = $title;
        }else if(!empty($title = setting_item("golf_page_search_banner"))) {
            $meta['seo_image'] = $title;
        }
        $meta['seo_desc'] = setting_item_with_lang("golf_page_list_seo_desc");
        $meta['seo_share'] = setting_item_with_lang("golf_page_list_seo_share");
        $meta['full_url'] = url()->current();
        return $meta;
    }

    public function terms(){
        return $this->hasMany($this->golfTermClass, "target_id");
    }
    ...
    public function getDetailUrl($include_param = true)
    {
        ...
        $urlDetail = app_get_locale(false, false, '/') . env('GOLF_ROUTE_PREFIX','golf') . "/" . $this->slug;
        if(!empty($param)){
    ...
    public static function getLinkForPageSearch( $locale = false , $param = [] ){
        ...
        return url(app_get_locale(false , false , '/'). env('GOLF_ROUTE_PREFIX','golf')."?".http_build_query($param));
    }

    public function getEditUrl()
    {
        return url(route('golf.admin.edit',['id'=>$this->id]));
    }
    ...
    public function addToCart(Request $request)
    {
        ...
        $start_date = new \DateTime($request->input('start_date')." ".$request->input('time_slot_selected'));
        if (empty($start_date)) {
            return $this->sendError(__("Start date is not a valid date"));
        }

        if(!$this->checkBusyDate( $start_date->format('Y-m-d H:i:s') )){
            return $this->sendError(__("Start date is not a valid date"));
        }
        ...
        $availableBook = $this->getDataAvailableBooking( $request->input('start_date') , $request->input('time_slot_selected'));
        if(empty($availableBook)){
            return $this->sendError(__("Please select other time slot"));
        }

        $tmp_time_slot = $this->tmp_time_slot;

        $max_guest = $total_tickets = $request->input('max_guest');
        $price = !empty($tmp_time_slot['price']) ? $tmp_time_slot['price']: $availableBook['price'];
        $cart_price = !empty($tmp_time_slot['cart_price']) ? $tmp_time_slot['cart_price']: $availableBook['cart_price'];
        $cart_sharing_price = !empty($tmp_time_slot['cart_sharing_price']) ? $tmp_time_slot['cart_sharing_price']: $availableBook['cart_sharing_price'];
        //$slot_book = $request->input('price_rows');
        $cart_type_selected = $request->input('cart_type_selected');

        $price_rows = [];
        for ($k = 1; $k <= $max_guest; $k++) {
            if ($cart_type_selected['type'] == 'sharing' && $k >= $cart_type_selected['number']) {
                $cart_price = $cart_sharing_price;
            }
            $price_rows[] = [
                'player' => 'Player ' . $k,
                'rate' => $price,
        ...
        $total_buyer_fee = 0;
        if (!empty($list_buyer_fees = setting_item('golf_booking_buyer_fees'))) {
        ...
        if ($check) {
            ...
            $booking->addMeta('price_rows', $price_rows);
            $booking->addMeta('max_guest', $max_guest);
            $booking->addMeta('cart_price', $cart_price);
            $booking->addMeta('cart_sharing_price', $cart_sharing_price);
            $booking->addMeta('cart_type_selected', $cart_type_selected);
            $booking->addMeta('time_slot', $tmp_time_slot);
            ...
            $bookingTimeSlot->start_time = $tmp_time_slot['time'];
            $type_time =  $this->duration_unit == "hour" ? HOUR_IN_SECONDS : MINUTE_IN_SECONDS;
            $end_time = strtotime($tmp_time_slot['time']) + $type_time * $this->duration;
    ...
    public function getDataAvailableBooking($start_date ,$slot )
    {
        $golfDate = $this->golfDateClass::where('target_id', $this->id)->where('start_date', $start_date)->first();
        if(empty($golfDate) or $golfDate->active == 0){
            return [];
        }
        $time_slot = collect($golfDate->time_slot);
        if(!count($time_slot)) return false;
        $this->tmp_time_slot = $time_slot->where('time',$slot)->first();
        if(empty($this->tmp_time_slot)) return false;
        return $golfDate->toArray();
    }
    ...
    public function getDataTimeSlotsAvailableBooking($start_date)
    {
        ...
        $golfDate = $this->golfDateClass::where('target_id', $this->id)->where('start_date', $start_date)->first();
        if(!empty($golfDate) and $golfDate->active == 0){
            return [];
        }
    ...
    {
        if(!$this->checkBusyDate( $booking->start_date )){
            return $this->sendError(__("Start date is not a valid date"));
        }
        return null;
    }
    ...
    public function getBookingData()
    {
        ...
        $list_fees = setting_item_array('golf_booking_buyer_fees');
        if(!empty($list_fees)){
    ...
    {
        return setting_item("golf_enable_review", 0);
    }

    public function getReviewApproved()
    {
        return setting_item("golf_review_approved", 0);
    }

    public function review_after_booking(){
        return setting_item("golf_enable_review_after_booking", 0);
    }

    public function count_remain_review()
    {
        $status_making_completed_booking = [];
        $options = setting_item("golf_allow_review_after_making_completed_booking", false);
        if (!empty($options)) {
    ...
    {
        $reviewStats = [];
        if (!empty($list = setting_item("golf_review_stats", []))) {
    ...
    {
        $golf_id = $this->id;
        $list_score = Cache::rememberForever('review_'.$this->type.'_' . $golf_id, function () use ($golf_id) {
            $dataReview = $this->reviewClass::selectRaw(" AVG(rate_number) as score_total , COUNT(id) as total_review ")->where('object_id', $golf_id)->where('object_model', "golf")->where("status", "approved")->first();
    ...
    public function getReviewList(){
        return $this->reviewClass::select(['id','title','content','rate_number','author_ip','status','created_at','vendor_id','author_id'])->where('object_id', $this->id)->where('object_model', 'golf')->where("status", "approved")->orderBy("id", "desc")->with('author')->paginate(setting_item('golf_review_number_per_page', 5));
    }
    ...
    public function getNumberServiceInLocation($location)
    {
        ...
        if ($number > 1) {
            return __(":number Golfs", ['number' => $number]);
        }
        return __(":number Golf", ['number' => $number]);
    }
    ...
    public function saveCloneByID($clone_id){
        ...
        foreach ($selected_terms as $term_id) {
            $this->golfTermClass::firstOrCreate([
                'term_id' => $term_id,
                'target_id' => $new->id
            ]);
        }
        //Language
        $langs = $this->golfTranslationClass::where("origin_id",$old->id)->get();
        if(!empty($langs)){
    ...
    public static function getServiceIconFeatured(){
        return "icofont-world";
    }

    public static function isEnable(){
        return setting_item('golf_disable') == false;
    }
    ...
    public function isDepositEnable(){
        return (setting_item('golf_deposit_enable') and setting_item('golf_deposit_amount'));
    }
    public function getDepositAmount(){
        return setting_item('golf_deposit_amount');
    }
    public function getDepositType(){
        return setting_item('golf_deposit_type');
    }
    public function getDepositFomular(){
        return setting_item('golf_deposit_fomular','default');
    }
    ...
    public static function isEnableEnquiry(){
        if(!empty(setting_item('booking_enquiry_for_golf'))){
            return true;
        }
        return false;
    }
    public static function isFormEnquiryAndBook(){
        $check = setting_item('booking_enquiry_for_golf');
        if(!empty($check) and setting_item('booking_enquiry_type_golf') == "booking_and_enquiry" ){
            return true;
        }
        return false;
    }
    public static function getBookingEnquiryType(){
        $check = setting_item('booking_enquiry_for_golf');
        if(!empty($check)){
            if( setting_item('booking_enquiry_type_golf') == "only_enquiry" ) {
                return "enquiry";
            }
        }
        return "book";
    ...
    {
        $model_golf = parent::query()->select("bravo_golfs.*");
        $model_golf->where("bravo_golfs.status", "publish");
        if (!empty($location_id = $request['location_id'] ?? "" )) {
            $location = Location::where('id', $location_id)->where("status","publish")->first();
            if(!empty($location)){
                $model_golf->join('bravo_locations', function ($join) use ($location) {
                    $join->on('bravo_locations.id', '=', 'bravo_golfs.location_id')
        ...
        if (!empty($price_range = $request['price_range'] ?? "")) {
            ...
            $raw_sql_min_max = "( (IFNULL(bravo_golfs.sale_price,0) > 0 and bravo_golfs.sale_price >= ? ) OR (IFNULL(bravo_golfs.sale_price,0) <= 0 and bravo_golfs.price >= ? ) )
                            AND ( (IFNULL(bravo_golfs.sale_price,0) > 0 and bravo_golfs.sale_price <= ? ) OR (IFNULL(bravo_golfs.sale_price,0) <= 0 and bravo_golfs.price <= ? ) )";
            $model_golf->WhereRaw($raw_sql_min_max,[$pri_from,$pri_from,$pri_to,$pri_to]);
        }
        ...
        if (is_array($terms) and !empty($terms = array_filter(array_values($terms)))) {
            foreach ($terms as $index=>$termId){
                $model_golf->join('bravo_golf_term as tt'.$index, function($join) use ($termId,$index){
                    $join->on('tt'.$index.'.target_id', "bravo_golfs.id");
        ...
        if (is_array($review_scores) && !empty($review_scores)) {
            ...
            foreach ($review_scores as $number){
                $where_review_score[] = " ( bravo_golfs.review_score >= ? AND bravo_golfs.review_score <= ? ) ";
                $params[] = $number;
                $params[] = $number.'.9';
            }
            $sql_where_review_score = " ( " . implode("OR", $where_review_score) . " )  ";
            $model_golf->WhereRaw($sql_where_review_score,$params);
        }

        if(!empty( $service_name = $request["service_name"] ?? "" ))
        {
            if( setting_item('site_enable_multi_lang') && setting_item('site_locale') != app()->getLocale() ){
                $model_golf->leftJoin('bravo_golf_translations', function ($join) {
                    $join->on('bravo_golfs.id', '=', 'bravo_golf_translations.origin_id');
                });
                $model_golf->where('bravo_golf_translations.title', 'LIKE', '%' . $service_name . '%');

            }
            else
            {
                $model_golf->where('bravo_golfs.title', 'LIKE', '%' . $service_name . '%');
            }
        }
        ...
        if(!empty($lat = $request['map_lat'] ?? "") and !empty($lgn = $request['map_lgn'] ?? "") and !empty($request['map_place']))
        {
            ...
            $distance  = setting_item('golf_location_radius_value',0);
            if(!empty($distance) and setting_item('golf_location_search_style')=='autocompletePlace'){
                $distanceType = setting_item('golf_location_radius_type',3959);
                ...
                $model_golf->whereRaw($string,[$distanceType,$lat,$lgn,$lat,$distance]);
            }

//            ORDER BY (POW((lon-$lon),2) + POW((lat-$lat),2))";
            $model_golf->orderByRaw("POW((bravo_golfs.map_lng-?),2) + POW((bravo_golfs.map_lat-?),2)",[$lgn,$lat]);
        }

        if(!empty($request['is_featured']))
        {
            $model_golf->where('bravo_golfs.is_featured',1);
        }
        if (!empty($request['custom_ids'])) {
            $model_golf->whereIn("bravo_golfs.id", $request['custom_ids']);
        }

        $orderby = $request["orderby"] ?? "";
        switch ($orderby){
            case "price_low_high":
                $raw_sql = "CASE WHEN IFNULL( bravo_golfs.sale_price, 0 ) > 0 THEN bravo_golfs.sale_price ELSE bravo_golfs.price END AS tmp_min_price";
                $model_golf->selectRaw($raw_sql);
                $model_golf->orderBy("tmp_min_price", "asc");
                break;
            case "price_high_low":
                $raw_sql = "CASE WHEN IFNULL( bravo_golfs.sale_price, 0 ) > 0 THEN bravo_golfs.sale_price ELSE bravo_golfs.price END AS tmp_min_price";
                $model_golf->selectRaw($raw_sql);
                $model_golf->orderBy("tmp_min_price", "desc");
                break;
            case "rate_high_low":
                $model_golf->orderBy("review_score", "desc");
                break;
            default:
                if(!empty($request['order']) and !empty($request['order_by'])){
                    $model_golf->orderBy("bravo_golfs.".$request['order'], $request['order_by']);
                }else{
                    $model_golf->orderBy("is_featured", "desc");
                    $model_golf->orderBy("id", "desc");
                }
        }

        $model_golf->groupBy("bravo_golfs.id");

        $max_guests = (int)( ($request['adults'] ?? 0) + ($request['children'] ?? 0));
        if($max_guests){
            $model_golf->where('max_guests','>=',$max_guests);
        }
        return $model_golf->with(['location','hasWishList','translation']);
    }
    ...
    public function dataForApi($forSingle = false){
        ...
        if($forSingle){
            ...
            $data['booking_fee'] = setting_item_array('golf_booking_buyer_fees');
    ...
    {
        return "\Themes\Golf\Golf\Controllers\AvailabilityController";
    }
    ...
    static public function getFiltersSearch()
    {
        ...
        return [
            ...
            [
                ...
                "field"    => "terms",
                "position" => "3",
                "data" => Attributes::getAllAttributesForApi("golf")
            ]
        ];
    }

    public static function getBookingType(){
        return setting_item('golf_booking_type','ticket');
    }
    ...
    {
        $search_fields = setting_item_array('golf_search_fields');
     ...
     public function generateTimeSlots($excludes = []){
        $slots = [];
        $periods = periodDate('2023-04-08 06:00','2023-04-08 18:00',false,$this->intervalTimeSlot.' minute');
        foreach ($periods as $dt){
            $slot = $dt->format('H:i');
            if(!in_array($slot,$excludes)){
                $slots[] = $slot;
            }
        }
        return $slots;
